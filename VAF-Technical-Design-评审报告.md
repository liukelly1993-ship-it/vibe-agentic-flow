...
| 测试与可验证性 | 9/10 | 42 个 unittest 覆盖主要 M0 风险 |
...
2. **验证命令已由 manifest 驱动，但仍是 M0 的受限实现。** 当前 `verify` 读取 `.vaf/manifest.yaml` 的默认命令和网络模式，并将命令规格纳入幂等键；命令仍必须匹配内置 PolicyEngine 白名单，超时和多命令编排留待后续版本。
3. **事件与投影仍是本地文件实现。** 多进程并发、远程不可变存储、事件 schema migration 和跨机器恢复需要平台化设计。
4. **Trace 仍是显式映射。** 后续可引入 Agent 候选链接，但必须经过 Schema、哈希和确定性策略质量门确认。
