...
VAF 的长期目标是实现从需求到生产部署的全流程自动化，但不把“无人监督生成并上线代码”作为产品目标。VAF 的核心价值是：让 AI 在明确规则、阶段门、工具权限和人工授权内工作，并让每一个结论和外部动作都可验证、可追责、可回滚。
...
- 通过阶段门和审批将合格制品交付到 staging 和 production。
...
4. 用户通过 `vaf review` 查看产物，执行 `approve` 或 `reject`。
5. PRD 批准后，VAF 依次生成技术方案、测试用例和实施任务，并在规定阶段暂停审批。
6. VAF 创建隔离 worktree，在允许路径内生成代码和单元测试。
7. VAF 通过策略网关执行白名单构建、Lint 和 pytest 命令。
8. 验证通过后，VAF 生成 Trace Report 和变更摘要，等待用户最终评审。
...
系统应允许用户查看当前待审产物，以带评论的批准、要求修改或拒绝结束评审。

验收条件：

- AC-023：审批记录包含 actor、时间、目标产物哈希和意见。
- AC-024：对旧版本提交审批不会批准新版本。
- AC-025：要求修改后，原审批记录与旧产物仍可审计。
...
vaf approve --run RUN-001 --comment "范围与验收条件确认"
vaf reject --run RUN-001 --comment "补充异常场景"
...
- Markdown/YAML 规格、审批结论和 TraceLink：以 Git 中不可变版本为事实来源。
...
- NFR-003：审批、工具结果和未来 CI 回调均支持重复投递。
...
| 版本 | 核心交付 | 完成定义 |
|---|---|---|
| M0 / 内核验证 | 领域模型、状态机、事件、Schema、模拟执行器 | 无 LLM 情况下通过审批、驳回、重试、恢复、失效和策略拒绝夹具 |
...
| 需求幻觉 | 实现了用户没要求的行为 | 假设清单、待确认项、PRD 审批门、禁止隐式扩展 |
...
4. PRD、技术方案和实施计划分别在哪些节点必须人工审批；建议 v0.1 至少审批 PRD 和最终变更。
