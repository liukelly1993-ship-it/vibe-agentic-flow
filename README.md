...
- 已验证：27 个 unittest 全部通过，并覆盖审批推进、事件链校验、跨进程幂等恢复、隔离 worktree、代码范围校验和回归验证。
- 已实现：事件日志投影与状态恢复、跨进程 ToolGateway 幂等复用、可注入 `AgentPort`、确定性 FakeAgent、`implement` 隔离代码写入和 worktree 内验证。
- 当前边界：真实 LLM、语义 TraceLink、自动回归选择、CI/CD staging/production 部署尚未接入；FakeAgent 只按显式 `implementation.changes` 文件计划生成候选代码。
- 下一步：补齐代码/任务/测试的语义 TraceLink 与覆盖率门，再接入真实 Agent Provider 和 CI/CD Adapter。
...
python3 -m vaf.cli --path /path/to/your/repo approve --run RUN-... --actor reviewer
...
例如：`run ... --implementation-spec implementation.yaml`。审批完实施计划后执行 `implement`，VAF 会在隔离 worktree 中通过 Policy Gateway 写入声明文件；`verify` 随后在该 worktree 中执行白名单命令，结果和截断后的 stdout/stderr 会写入运行事件日志。
