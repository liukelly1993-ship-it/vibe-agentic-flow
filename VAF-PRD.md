...
在单一生成任务内，以 Vue 3 + Vite、FastAPI、PostgreSQL（PRD 明确要求时允许 MySQL）为首个交付基线，完成一条需求到可验证代码的闭环：
...
- 不支持 Java 等其他后端语言，也不生成 React、Next.js 等其他前端框架。
...
1. 用户上传 Markdown、PDF、DOCX、HTML、TXT 或公共飞书 PRD，并按需指定本地知识库目录。
2. VAF 对原始 PRD 执行准入评审；分数必须严格大于 90，且不得存在 P0 阻塞。
3. PRD 引用既有系统、内部规则、外部契约、受监管数据或未附资料时，VAF 要求知识库并固化来源哈希；缺失时停止生成。
4. 准入通过后创建 Change，生成结构化 PRD 草稿，并输出假设、缺口和待确认项。
5. VAF 通过 `review` 输出评分和证据；`autopilot` 在分数严格大于 90 且无 P0 阻塞时自动推进，否则回到当前阶段重试。
6. PRD 门通过后，VAF 依次生成技术方案、测试用例和实施任务，不等待人工审批。
7. VAF 创建隔离 worktree，在允许路径内生成代码和单元测试。
8. VAF 通过策略网关执行白名单构建、Lint 和测试命令。
9. 验证通过后，VAF 生成 Trace Report、Docker Compose 候选和变更摘要；质量门通过后生成本地可交付候选，不自动合并生产分支。
10. 用户自行决定是否把候选变更提交、创建 PR 或继续修改；v0.1 不自动合并。
...
#### FR-004A 原始 PRD 准入与知识依据（P0）

系统应在创建项目、生成下游文档或代码之前，对用户上传的原始 PRD 进行独立准入评审。评审覆盖来源可读性、目标与价值、用户场景、范围、功能需求、验收条件、原型、数据接口、非功能要求、风险假设和知识依据。

验收条件：

- AC-014A：准入分数必须严格大于 90，且没有 P0 finding，才能创建生成项目。
- AC-014B：原型图片、页面截图或线框图是 P0；普通 Logo、商品图片或无关插图不能作为原型证据。
- AC-014C：引用既有系统、内部规则、外部接口、受监管数据或其他资料时，必须提供相关本地知识库或等价可信来源。
- AC-014D：知识库只能从显式允许根目录读取，并记录逐文件哈希和快照哈希；Agent 自声明不能替代来源证据。
- AC-014E：通用、自包含且不依赖外部事实的 PRD 不强制提供知识库。
...
- NFR-007A：本地知识库路径必须位于管理员配置的允许根目录内，禁止符号链接逃逸和越界读取。
...
| v0.1 | PRD 准入、知识依据、Vue/FastAPI、worktree、代码/测试、Docker Compose 候选、验证、Trace | 单仓库 Golden Change 稳定完成需求到本地可验证代码 |
...
- 生成项目的首个前端固定为 Vue 3 + Vite；数据库默认 PostgreSQL，只有 PRD 明确要求时使用 MySQL。
- 本地交付以 Docker Compose 为正式契约；Docker 不可用时不得把静态文件检查描述为运行验收通过。
